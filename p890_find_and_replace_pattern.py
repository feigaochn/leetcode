...
b -> e, ...}.  "ccc" does not match the pattern because {a -> c, b -> c, ...}
...
class Solution:
    def findAndReplacePattern(self, words, pattern):
        ...
        :rtype: List[str]
        """
        lp = len(set(list(pattern)))

        def matched(word):
            d = dict(zip(word, pattern))
            w2 = "".join(d[c] for c in word)
            return w2 == pattern and len(set(list(word))) == lp

        return [w for w in words if matched(w)]


sol = Solution().findAndReplacePattern
print(sol(words=["abc", "deq", "mee", "aqq", "dkd", "ccc"], pattern="abb"))
