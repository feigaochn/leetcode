...
An email address starts with a name, followed by the symbol '@', followed by a
name, followed by the dot '.' and followed by a name.
...
To mask a phone number with country code like "+111 111 111 1111", we write it
in the form "+***-***-***-1111".  The '+' sign and the first '-' sign before
the local number should only exist if there is a country code.  For example, a
12 digit phone number mask should start with "+**-".

Note that extraneous characters like "(", ")", " ", as well as extra dashes or
plus signs not part of the above formatting scheme should be removed.
...
class Solution:
    ...
    def maskPII(self, S):
        ...
        def go_email(s):
            name = "([a-zA-Z]{2,})"
            names = re.findall(r"{0}@{0}\.{0}".format(name), s.lower())[0]
            return "{}*****{}@{}.{}".format(
                names[0][0], names[0][-1], names[1], names[2]
            )
        ...
        def go_phone(s):
            ...
            if len(digits) == 10:
                ...
                return "+{}-***-***-{}".format("*" * (len(digits) - 10), digits[-4:])
