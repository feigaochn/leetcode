...
money than 0.  The only person who is quieter (has lower quiet[x]) is person 7,
...
class Solution:
    ...
    def loudAndRich(self, richer, quiet):
        ...
        outbounce = defaultdict(set)
        for x, y in richer:
            outbounce[x].add(y)

        while outbounce:
            keys = outbounce.keys()
            values = reduce(set.union, outbounce.values(), set())
            front = keys - values
            for p in front:
                for q in outbounce[p]:
